...
export default class ProductForm {
  ...
  get template() {
    ...
    <form data-element="productForm" class="form-grid">
      ...
      <div class="form-group form-group__wide" data-element="sortable-list-container">
        ...
        <div data-element="imageListContainer"><ul class="sortable-list"></ul>
        </div>
  ...
  init() {
    ...
    this.subElements.imageListContainer.addEventListener('click', this.deleteImgElement);
  ...
  deleteImgElement = (event) => {
    if (!event.target.hasAttribute('data-delete-handle')) {
      return;
    }
    let index = 0;
    const closestLiElement = event.target.closest('li');
    for (const iterator of closestLiElement.parentElement.children) {
      if (iterator === closestLiElement) {
        break;
      }
      index += 1;
    }
  ...
  uploadFile = async (event) => {
    ...
    try {
      ...
      const newImgItem = { url: response.data.link, source: event.target.files[0].name };
      this.data.images.push(newImgItem);
      this.subElements.imageListContainer.firstElementChild.insertAdjacentHTML('beforeend', this.getImgItemText(newImgItem));
  ...
  fillFormData(data) {
    ...
    this.subElements.imageListContainer.firstElementChild.innerHTML = this.data.images.map(value => {
